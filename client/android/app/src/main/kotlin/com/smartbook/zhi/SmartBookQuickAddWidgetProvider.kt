 ...
 *
 * 尺寸判定机制同 [SmartBookNetWorthWidgetProvider]:本 provider 类名覆盖
 ...
 */
open class SmartBookQuickAddWidgetProvider : HomeWidgetProvider() {
    companion object {
        private const val TAG = "SmartBookQuickAddWidget"
        ...
        appWidgetIds.forEach { widgetId ->
            try {
                ...
                val views = RemoteViews(context.packageName, R.layout.quick_add_widget).apply {
                    ...
                    // smartbook://new?type=expense&category=<id>。
                    try {
                        val intent = createLaunchIntentWithDeepLink(context, "smartbook://new?type=expense")
