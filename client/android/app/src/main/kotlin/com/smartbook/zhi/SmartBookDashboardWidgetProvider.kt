 ...
 * [SmartBookNetWorthWidgetProvider] 那样按 `getAppWidgetOptions` 分档。
 */
class SmartBookDashboardWidgetProvider : HomeWidgetProvider() {
    companion object {
        private const val TAG = "SmartBookDashboardWidget"
        ...
        appWidgetIds.forEach { widgetId ->
            try {
                ...
                val views = RemoteViews(context.packageName, R.layout.dashboard_widget).apply {
                    ...
                    // 记账行 → 记支出。
                    try {
                        val detailIntent = createLaunchIntentWithDeepLink(context, "smartbook://open?page=detail")
                        ...
                        val expenseIntent = createLaunchIntentWithDeepLink(context, "smartbook://new?type=expense")
