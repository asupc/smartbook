 ...
 * [resolveImageKey]。尺寸判定机制同 [SmartBookNetWorthWidgetProvider]:
 * Dart 侧 `matchInstalledAll` 为命中类名渲染该类型全部尺寸的图,任意缩放
 * 档位都有现成图可显。
 */
open class SmartBookRecentWidgetProvider : HomeWidgetProvider() {
    companion object {
        private const val TAG = "SmartBookRecentWidget"
        ...
        appWidgetIds.forEach { widgetId ->
            try {
                ...
                val views = RemoteViews(context.packageName, R.layout.recent_widget).apply {
                    ...
                    // 并携带交易 id,例如 smartbook://open?page=detail&id=<id>。
                    try {
                        val intent = createLaunchIntentWithDeepLink(context, "smartbook://open?page=detail")
