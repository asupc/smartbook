...
///
/// 把 SmartBookCloudProvider 的 invites / members API 封装成可缓存的
...
final ledgerMembersProvider = FutureProvider.autoDispose
    .family<List<SmartBookCloudLedgerMember>, String>((ref, ledgerId) async {
...
final memberStatsProvider = FutureProvider.autoDispose
    .family<SmartBookCloudMemberStats?, MemberStatsKey>((ref, key) async {
  ref.watch(sharedResourceRefreshProvider);
  final cloud = await ref.watch(smartbookCloudProviderInstance.future);
...
final ledgerInvitesProvider = FutureProvider.autoDispose
    .family<List<SmartBookCloudInvite>, String>((ref, ledgerId) async {
  final cloud = await ref.watch(smartbookCloudProviderInstance.future);
  if (cloud == null) return const [];
  try {
...
Future<SmartBookCloudInvite> createInviteAndRefresh(
  WidgetRef ref, {
  ...
  if (cloud == null) {
    throw StateError('SmartBook Cloud not configured');
  }
