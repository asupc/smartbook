...
// SmartBook Cloud 后一岁星座主题色一直闪」的成因。
//
...
final noteHistoryPreferencesInitProvider = FutureProvider<void>((ref) async {
  ...
  ref.listen<NoteHistoryScope>(noteHistoryScopeProvider, (prev, next) async {
    // 用户选择变化后写本机偏好，并在 SmartBook Cloud 模式下同步。
  ...
  ref.listen<int>(noteHistoryLimitProvider, (prev, next) async {
    // 用户修改数量后写本机偏好，并在 SmartBook Cloud 模式下同步。
...
// 本地持久化 + 并入 appearance 包,随 SmartBook Cloud 多设备同步。
...
/// 的当前值打包推给 server 的 /profile/me。非 SmartBook Cloud 模式 provider
...
final incomeExpenseColorSchemeInitProvider = FutureProvider<void>((ref) async {
  ...
  ref.listen<bool>(incomeExpenseColorSchemeProvider, (prev, next) async {
    ...
    // SmartBook Cloud 模式下把配色偏好推给 server；web 端会通过 WS
...
// 用户显示名(昵称)。本地真值存 prefs 'displayName';SmartBook Cloud 模式下改动
...
/// 把显示名推给 server 的 /profile/me(仅 SmartBook Cloud 模式)。非 cloud 模式
