...
/// 用户主币种(大写 ISO code)。本地真值存 prefs 'baseCurrency';SmartBook Cloud
...
/// 把主币种推给 server 的 /profile/me(仅 SmartBook Cloud 模式)。非 cloud 模式
...
Future<bool> _fetchAndStoreRatesForBase({
  ...
  try {
    ...
    try {
      final cloudProvider = await readFuture(smartbookCloudProviderInstance);
