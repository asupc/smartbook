...
// SmartBook Cloud 配置(不管是否激活)
final smartbookCloudConfigProvider =
    FutureProvider<CloudServiceConfig?>((ref) async {
  final store = ref.watch(cloudServiceStoreProvider);
  return store.loadSmartBookCloud();
});
...
final syncServiceProvider = Provider<SyncService>((ref) {
  ...
  // SmartBook Cloud → SyncEngine（增量同步）
  if (config.type == CloudBackendType.smartbookCloud) {
    ...
    AIProviderManager.onConfigChanged = () {
      unawaited(() async {
        try {
          final cloud = await ref.read(smartbookCloudProviderInstance.future);
    ...
    if (currentLedgerId > 0 && !_autoSyncInProgress) {
      ...
      Future(() async {
        try {
          ...
          // SmartBook Cloud 的 apply 是按 entity_sync_id upsert 幂等的，重放
          // 安全。
          if (newLedgerCount > 0) {
            try {
...
/// 已初始化的 SmartBookCloudProvider 实例
/// 用于 SyncEngine 和其他需要直接访问 SmartBook Cloud API 的场景
final smartbookCloudProviderInstance =
    FutureProvider<SmartBookCloudProvider?>((ref) async {
  final configAsync = ref.watch(activeCloudConfigProvider);
  if (!configAsync.hasValue) return null;

  final config = configAsync.value!;
  if (!config.valid || config.type != CloudBackendType.smartbookCloud) {
    return null;
  }

  try {
    final services = await createCloudServices(config);
    if (services.provider is! SmartBookCloudProvider) return null;
    final provider = services.provider as SmartBookCloudProvider;

    final email = config.smartbookCloudEmail;
    final password = config.smartbookCloudPassword;
    ...
    // 重建。
    if (services.auth is SmartBookCloudAuthService) {
      (services.auth as SmartBookCloudAuthService).setRecoveryCredentials(
        email: email,
        password: password,
      );
    }
...
/// null,UI 自己隐藏。非 SmartBook Cloud 模式直接 null。
///
...
final smartbookCloudServerVersionProvider =
...
Future<void> reconcileProfileToServer({
  required Future<SmartBookCloudProvider?> cloudProviderFuture,
...
// 这就是「开启 SmartBook Cloud 时主题色闪烁」。绑定配色的皮肤会闪,
...
final cloudMyProfileProvider =
    FutureProvider<SmartBookCloudProfile?>((ref) async {
  ref.watch(syncStatusRefreshProvider);
...
/// 远程账本列表（慢速，网络请求）
///
/// 调用 SmartBook Cloud 的 `/read/ledgers` 取 server 上当前用户所有账本，再
