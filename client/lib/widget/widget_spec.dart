...
class WidgetSpec {
  ...
  /// (`SmartBookWidgetBundle.swift`)注册的类型才有值;未注册类型此字段为
  ...
  /// 入口,见 `SmartBookSizedWidgetProviders.kt`)。所有入口都继承父类"按
  ...
  /// `SmartBookWidget.swift` / Android `SmartBookWidgetProvider.kt` 原生壳
  ...
  // ---- 收支速览(glance):小/中 ----
  /// 小号(补全新增):iOS 挂在**现有** kind `SmartBookWidget` 的 systemSmall
  ...
  static const glanceSmall = WidgetSpec._(
    ...
    iosKind: 'SmartBookWidget',
    iosFamily: 'systemSmall',
    androidClassName:
        'com.smartbook.zhi.SmartBookGlanceSmallWidgetProvider',
  );

  /// 现有唯一已上线的组件:中号收支速览。原生标识与升级前完全一致
  /// (iOS kind `SmartBookWidget` / Android provider 类名
  /// `SmartBookWidgetProvider`),存量桌面放置靠这两个标识存活,不可更改。
  static const glanceMedium = WidgetSpec._(
    ...
    androidClassName: 'com.smartbook.zhi.SmartBookWidgetProvider',
  );

  // ---- 净资产(netWorth):小/中/大 ----
  // iOS 原生壳见 ios/SmartBookWidget/SmartBookNetWorthWidget.swift
  // (kind SmartBookNetWorthWidget,supportedFamilies 小/中/大)。
  static const netWorthSmall = WidgetSpec._(
    type: HWType.netWorth,
    size: HWSize.small,
    logicalSize: Size(155, 155),
    iosKind: 'SmartBookNetWorthWidget',
    iosFamily: 'systemSmall',
    androidClassName: 'com.smartbook.zhi.SmartBookNetWorthWidgetProvider',
    androidExtraClassNames: [
      'com.smartbook.zhi.SmartBookNetWorthMediumWidgetProvider',
      'com.smartbook.zhi.SmartBookNetWorthLargeWidgetProvider',
    ],
  );
  ...
  // ---- 快速记账(quickAdd):小/中 ----
  // iOS 原生壳见 ios/SmartBookWidget/SmartBookQuickAddWidget.swift
  // (kind SmartBookQuickAddWidget,supportedFamilies 小/中)。
  static const quickAddSmall = WidgetSpec._(
    type: HWType.quickAdd,
    size: HWSize.small,
    logicalSize: Size(155, 155),
    iosKind: 'SmartBookQuickAddWidget',
    iosFamily: 'systemSmall',
    androidClassName: 'com.smartbook.zhi.SmartBookQuickAddWidgetProvider',
    androidExtraClassNames: [
      'com.smartbook.zhi.SmartBookQuickAddMediumWidgetProvider',
    ],
  );
  ...
  // ---- 预算进度(budget):小/中 ----
  // iOS 原生壳见 ios/SmartBookWidget/SmartBookBudgetWidget.swift
  // (kind SmartBookBudgetWidget,supportedFamilies 小/中)。
  static const budgetSmall = WidgetSpec._(
    type: HWType.budget,
    size: HWSize.small,
    logicalSize: Size(155, 155),
    iosKind: 'SmartBookBudgetWidget',
    iosFamily: 'systemSmall',
    androidClassName: 'com.smartbook.zhi.SmartBookBudgetWidgetProvider',
    androidExtraClassNames: [
      'com.smartbook.zhi.SmartBookBudgetMediumWidgetProvider',
    ],
  );
  ...
  // ---- 最近交易(recent):中/大 ----
  // iOS 原生壳见 ios/SmartBookWidget/SmartBookRecentWidget.swift
  // (kind SmartBookRecentWidget,supportedFamilies 中/大)。
  static const recentMedium = WidgetSpec._(
    type: HWType.recent,
    size: HWSize.medium,
    logicalSize: Size(364, 169),
    iosKind: 'SmartBookRecentWidget',
    iosFamily: 'systemMedium',
    androidClassName: 'com.smartbook.zhi.SmartBookRecentWidgetProvider',
    androidExtraClassNames: [
      'com.smartbook.zhi.SmartBookRecentLargeWidgetProvider',
    ],
  );
  ...
  // ---- 综合仪表盘(dashboard):仅大 ----
  // iOS 原生壳见 ios/SmartBookWidget/SmartBookDashboardWidget.swift
  // (kind SmartBookDashboardWidget,supportedFamilies 仅大)。
  static const dashboardLarge = WidgetSpec._(
    type: HWType.dashboard,
    size: HWSize.large,
    logicalSize: Size(364, 382),
    iosKind: 'SmartBookDashboardWidget',
    iosFamily: 'systemLarge',
    androidClassName: 'com.smartbook.zhi.SmartBookDashboardWidgetProvider',
  );
  ...
  /// (**prod 商店包** `com.smartbook.zhi` 正是如此),它是带前导点的
  /// 短名 `.SmartBookWidgetProvider` 而**不是**全限定名;dev/debug 因
  ...
  /// `XxxSmartBookWidgetProvider` 误匹配成 `SmartBookWidgetProvider`。
