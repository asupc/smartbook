...
class VoiceBillingHelper {
  /// 启动语音记账
  ///
  /// [onResult] 可选:识别流程结束后回调(成功或失败都会回调,取消/权限拒绝
  /// 不回调)。传入后,对话框不再弹成功/失败 toast —— 由调用方(如 AI 对话页)
  /// 接手结果展示。注意被识别兜底(`PostProcessor.run`)仍在对话框内完成。
  static Future<void> startVoiceBilling(
    BuildContext context,
    WidgetRef ref, {
    Future<void> Function(BookkeepingResult result, String? recognizedText)?
        onResult,
  }) async {
...
class _VoiceRecordingDialog extends ConsumerStatefulWidget {
  ...
  /// 识别结束后回调（成功/失败都调）；传入时对话框不再弹结果 toast
  final Future<void> Function(BookkeepingResult result, String? recognizedText)?
      onResult;
  ...
  Future<void> _stopAndProcess() async {
    ...
    try {
      ...
      if (!response.result.success) {
        Navigator.of(context).pop();
        // 有回调时(如 AI 对话页)交给调用方展示结果,不弹 toast
        if (widget.onResult != null) {
          await widget.onResult!(response.result, response.recognizedText);
          return;
        }
