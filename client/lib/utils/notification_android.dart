...
class AndroidNotificationUtil implements util.NotificationUtil {
  ...
  /// 自启动/后台保活引导(M4):尝试跳厂商「自启动管理」设置页
  /// (vivo/OPPO/小米/华为),失败兜底应用详情页。返回是否发起跳转。
  Future<bool> requestAutoStartGuide() async {
    try {
      final result = await _channel.invokeMethod('requestAutoStartGuide');
      return result ?? false;
    } catch (e) {
      print('[Android] 自启动引导失败: $e');
      return false;
    }
  }
