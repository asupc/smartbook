...
class _BeeAppState extends ConsumerState<BeeApp>
  ...
  /// 这里是一次 ref.read，等 smartbookCloudProviderInstance 异步就绪再重建时没有
  /// 监听者，provider 内部的 auto-sync 块永远跑不到 —— 用户看到"app 启动没同步本地
  /// 数据到 SmartBook Cloud"。这里 listenManual 保持 provider 活跃，并在它从占位
  /// 对象变成真正的 SyncEngine 时主动触发一次 sync。
  void _refreshLedgersStatusInBackground() {
    // 冷启动时先 eager-await smartbookCloudProviderInstance 一次，强制让这个
    // FutureProvider 真正跑起来。否则只是"被定义"但没人读，
    // SmartBookCloudAuthService.initialize() 永远不会跑，session 不会从
    // SharedPreferences 恢复 —— 就是之前用户感受到的"必须打开配置保存才会
    // 登录"bug 的根因。后面的 listenManual 再做后续响应式逻辑。
    Future.microtask(() async {
      try {
        await ref.read(sp.smartbookCloudProviderInstance.future);
      } catch (_) {
        // 非 SmartBook Cloud 配置或初始化失败：忽略，让下面的 listenManual 兜住。
      }
    });
  ...
  void _triggerInitialCloudSync(SyncEngine engine) {
    ...
    Future(() async {
      try {
        ...
        logger.info('AppStart',
            'SmartBook Cloud 首次同步: 本地账本数=${ledgers.length}');
        ...
        logger.info('AppStart',
            'SmartBook Cloud 首次同步完成: synced=${ledgers.length - skipped} skipped=$skipped pushed=$totalPushed 总耗时 ${totalMs}ms');
        ref.read(syncStatusRefreshProvider.notifier).state++;
        ref.read(ledgerListRefreshProvider.notifier).state++;
      } catch (e, st) {
        logger.error('AppStart', 'SmartBook Cloud 首次同步异常', e, st);
      }
    });
  }
  ...
  /// 小组件「净资产 / 预算 / 最近交易」卡片点击 → `smartbook://open?page=` 的
