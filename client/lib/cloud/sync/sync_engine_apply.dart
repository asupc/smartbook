...
extension SyncEngineApplyExt on SyncEngine {
  ...
  Future<bool> applyRemoteChange(SmartBookCloudSyncChange change) async {
    // 跳过本设备自己的变更
  ...
  Future<void> _applyTransactionChange(SmartBookCloudSyncChange change) async {
  ...
  Future<void> _applyAccountChange(SmartBookCloudSyncChange change) async {
    final syncId = change.entitySyncId;
    // ledger_id 也按 syncId 映射到本地 int。account 表 ledgerId 是 legacy
    // 字段，但 insert 时仍需填个有效值；映射失败再 fallback 到旧格式。
    //
    // ⚠️ user-global 通道:server 对 scope='user' 的 account change 在 pull
    // 响应里把 ledger_id 填成 sentinel '__user_global__'(客户端 change 模型
    // 无 scope 字段,只能靠 ledgerId 区分)。sentinel 没有任何本地 ledger
    // 会命中,解析必然 miss —— 此前落 -1,账户管理页按账本过滤(== 当前账本)
    // 永远看不到云端账户。account 是 user-global 实体,不依附账本,统一落 0
    // (与 ChangeTracker.recordPulledFromServer 的 ledgerId=0 约定一致)。
    final ledgerIdInt = await _resolveLedgerIdBySyncId(change.ledgerId) ??
        int.tryParse(change.ledgerId) ??
        0;
  ...
  Future<void> _applyCategoryChange(SmartBookCloudSyncChange change) async {
    final syncId = change.entitySyncId;
  ...
  Future<void> _applyTagChange(SmartBookCloudSyncChange change) async {
    final syncId = change.entitySyncId;
  ...
  Future<void> _applyBudgetChange(SmartBookCloudSyncChange change) async {
    final syncId = change.entitySyncId;
  ...
  Future<void> _applyLedgerChange(SmartBookCloudSyncChange change) async {
    final syncId = change.entitySyncId;
    if (change.action == 'delete') {
