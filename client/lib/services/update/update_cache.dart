...
class UpdateCache {
  ...
  static Future<String?> checkCachedApkForUrl(String downloadUrl) async {
    try {
      ...
      // 从URL文件名提取版本号（格式如 smartbook-0.8.1.apk）
      String? version;
      final versionMatch = RegExp(r'smartbook-([0-9]+\.[0-9]+\.[0-9]+)\.apk')
          .firstMatch(fileName);
      ...
      // 在下载目录中查找对应版本的SmartBook APK
      // 文件名格式应该是 SmartBook_v{version}.apk
      final targetFileName = 'SmartBook_v$version.apk';
      ...
      if (await file.exists()) {
        ...
        for (final checkFile in files) {
          if (checkFile is File &&
              checkFile.path.contains('SmartBook') &&
