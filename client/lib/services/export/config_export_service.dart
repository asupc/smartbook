...
class ExportOptions {
  ...
  /// 是否把 SmartBook Cloud 的登录态（access/refresh token）一起导出。
  /// 默认 false —— 只导出 base_url + email，密码 / token 不写进 yaml。
  /// 测试或跨设备快速登录时显式勾选。
  final bool smartbookCloudCredentials;
  ...
  const ExportOptions({
    ...
    this.ai = true,
    this.smartbookCloudCredentials = false,
  });
...
class AppConfig {
  ...
  final S3Config? s3;
  final SmartBookCloudConfig? smartbookCloud;
  final AIConfig? ai;
  ...
  Map<String, dynamic> toYaml() {
    ...
    if (smartbookCloud != null) {
      map['smartbook_cloud'] = smartbookCloud!.toMap();
    }
  ...
  static AppConfig fromYaml(Map<dynamic, dynamic> yaml) {
    return AppConfig(
      ...
      s3: yaml.containsKey('s3')
          ...
          : null,
      smartbookCloud: yaml.containsKey('smartbook_cloud')
          ? SmartBookCloudConfig.fromMap(
              Map<String, dynamic>.from(yaml['smartbook_cloud'] as Map))
          : yaml.containsKey('beecount_cloud')
              // 兼容品牌更名前导出的旧键名(2026-09)
              ? SmartBookCloudConfig.fromMap(
...
/// SmartBook Cloud 配置（自部署 FastAPI 后端的 base URL + 可选登录态）
///
...
class SmartBookCloudConfig {
  ...
  static SmartBookCloudConfig fromMap(Map<String, dynamic> map) =>
      SmartBookCloudConfig(
...
class ConfigExportService {
  ...
  static Future<String> exportToYaml({
    ...
    // 读取 SmartBook Cloud 配置。base_url + email 总是导出（方便 B 设备导入
    // 快速填回登录表单）；access/refresh token 属于登录态，需 options 显式
    // 勾选才带上。当前实现：cloud_smartbook_cloud_cfg 里只存 base_url+email，
    // session token 另一把 SharedPreferences key 管 —— 导出 yaml 只取前者。
    SmartBookCloudConfig? smartbookCloudConfig;
    var smartbookCfgRaw = prefs.getString('cloud_smartbook_cloud_cfg');
    // 品牌更名前 key 为 cloud_beecount_cloud_cfg,读旧 key 兼容。
    if (smartbookCfgRaw == null) {
      smartbookCfgRaw = prefs.getString('cloud_beecount_cloud_cfg');
    }
    if (smartbookCfgRaw != null) {
      try {
        final cfg = decodeCloudConfig(smartbookCfgRaw);
        final baseUrl = cfg.smartbookCloudBaseUrl ?? '';
        if (baseUrl.isNotEmpty) {
          smartbookCloudConfig = SmartBookCloudConfig(
            baseUrl: baseUrl,
            email: cfg.smartbookCloudEmail,
            // 跟 Supabase 一样：如果用户在 mobile 勾过 "记住账号密码"，
            // smartbookCloudPassword 就会在 SharedPreferences 里，带上它方便
            // B 设备导入后无感登录。没勾就是 null，yaml 也不写这一行。
            password: cfg.smartbookCloudPassword,
            // access/refresh token 走独立 session storage（key 里带 baseUrl
            // sha1），跨设备迁移风险高，导出 yaml 不带。
          );
        }
      } catch (e) {
        logger.warning('ConfigExport', '读取 SmartBook Cloud 配置失败: $e');
      }
    }
    ...
    final exportSmartBookCloud =
        options.appSettings ? smartbookCloudConfig : null;
    ...
    final config = AppConfig(
      ...
      webdav: exportWebdav,
      s3: exportS3,
      smartbookCloud: exportSmartBookCloud,
      ai: exportAi,
    ...
    if (yamlMap.containsKey('smartbook_cloud')) {
      buffer.writeln('smartbook_cloud:');
      final bc = yamlMap['smartbook_cloud'] as Map<String, dynamic>;
      buffer.writeln('  # SmartBook Cloud 自部署后端配置');
  ...
  static Future<void> importFromYaml(
    ...
    // 导入 SmartBook Cloud 配置（base_url + 可选 email/password）。
    // 有 email+password 时跟 Supabase 一样，导入后 app 启动可自动登录；
    // 只有 email 时登录页预填邮箱，等用户输密码。
    if (options.appSettings && config.smartbookCloud != null) {
      final bcCfg = CloudServiceConfig(
        type: CloudBackendType.smartbookCloud,
        name: 'SmartBook Cloud',
        smartbookCloudBaseUrl: config.smartbookCloud!.baseUrl,
        smartbookCloudEmail: config.smartbookCloud!.email,
        smartbookCloudPassword: config.smartbookCloud!.password,
      );
      await prefs.setString(
          'cloud_smartbook_cloud_cfg', encodeCloudConfig(bcCfg));
      logger.info(
          'ConfigImport',
          'SmartBook Cloud 配置已导入 url=${config.smartbookCloud!.baseUrl} '
              'hasEmail=${config.smartbookCloud!.email != null} '
              'hasPassword=${config.smartbookCloud!.password != null}');
    }
