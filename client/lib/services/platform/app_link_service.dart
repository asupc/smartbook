...
enum AppLinkAction {
  ...
  /// `smartbook://open?page=assets|budget|detail`
  open,
...
/// - smartbook://new?type=expense - 手动记账（支出/收入）
/// - smartbook://new?type=expense&category=12 - 手动记账并预填分类（小组件
///   「快速记账」点分类格用，category 为分类 id）
/// - smartbook://add?amount=100&type=expense&category=餐饮 - 自动记账
/// - smartbook://open?page=assets|budget|detail - 打开指定页面（小组件点击
///   净资产/预算/最近交易卡片用）
...
class AppLinkService {
  ...
  static const EventChannel _eventChannel =
      EventChannel('com.smartbook.app_intents/events');

  /// iOS AppIntents 方法通道(回调 Swift,告知后台处理已完成可以放 perform 返回)
  static const MethodChannel _methodChannel =
      MethodChannel('com.smartbook.app_intents');
...
class AppLinkBuilder {
  static const String scheme = 'smartbook';
