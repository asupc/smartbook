...
/// 决定 transaction.type(与 [BillCreationService] 落库同一语义,顶层函数便于单测):
/// 1. [type] 显式给出 → 直接用(transfer/income/expense)
/// 2. [category] 含「转账/还款」字样 → transfer
///    (还款=账户间划转,不进消费统计 —— §6.1 口径;含"还款"从宽,兼容
///     "还信用卡"/"repayment" 等表达)
/// 3. 默认 expense(AI 模式下 amount 负值代表支出;若 type 漏了我们保守按
///    expense 处理,避免误记成收入)
String resolveTransactionType({BillType? type, String? category}) {
  if (type == BillType.transfer) return 'transfer';
  if (type == BillType.expense) return 'expense';
  if (type == BillType.income) return 'income';
  final cat = category?.trim() ?? '';
  if (cat.isEmpty) return 'expense';
...
class BillCreationService {
  ...
  Future<int?> createFromBill({
    ...
    logger.info(
      ...
      '时间:${_formatDateTime(happenedAt)} | 备注已脱敏 | '
  ...
  /// 解析 AI 分类名 → 本地分类 id(与入账路径同一套 完全→模糊→规则 匹配)。
  /// M3 消费提醒引擎在分流前用它做分类窗口判定;匹配不上返回 null(该窗口跳过,
  /// 不影响其他规则)。
  Future<int?> matchCategoryId(BillInfo bill) async {
    final type = _resolveType(bill);
    final categories = await _loadUsableCategories(type);
  ...
  /// 决定 transaction.type(委托给顶层 [resolveTransactionType],便于单测)。
  ...
  /// 收入/支出场景的账户匹配。
  ///
  /// 优先级:AI 账户名(完全→模糊→类型映射)→ 来源渠道映射(M4)> 默认账户。
  Future<int?> _matchAccount(
    ...
    // 渠道映射仅在 AI 未给账户名时生效(给了名字且能匹配就尊重 AI)。
    if (sourceChannel != null &&
        (aiAccountName == null || aiAccountName.isEmpty)) {
      final rule = await ChannelAccountStore().ruleFor(sourceChannel);
      if (rule != null) {
        logger.debug(
            _tag, '[账户匹配-渠道] 来源"$sourceChannel" → 账户(映射,ID:${rule.accountId})');
