...
class GenericBillParser implements BillParser {
  ...
  Map<String, int> mapColumns(List<String> headerRow) {
    ...
    // 高优先级字段先扫一遍(第一遍占位,不会被同语义的后到列顶替):
    // - "收/支"(新版支付宝表头里的收支方向列)必须先于"类型"占位 type,
  ...
  /// 在 [headerRow] 中找第一个包含任一 [keywords] 的列并映射为 [field]。
  void _tryMapByKeywords(
    ...
    for (int i = 0; i < headerRow.length; i++) {
      if (keywords.any((k) => headerRow[i].contains(k))) {
        mapping[field] = i;
        return;
      }
    }
  ...
  String? _normalizeToKey(String raw) {
    ...
    // 再匹配收支类型字段。
    // 注意优先级:新版支付宝表头同时有"类型"(即时到账交易等渠道说明)和
    // "收/支"(真正的收支方向),"收/支"必须优先映射为 type,否则先出现的
    // "类型"列把 type 占走,导入全部按默认类型归类。
    if (noSpace == '收/支' ||
        noSpace == '收支' ||
        noSpace == '收支出' ||
        noSpace == '收/支出') {
    ...
    if (_containsAny(s, ['类型', '方向'])) {
      return 'type';
    }
    // 描述字段:商品名称/商品说明优先映射为 note(新版支付宝"商品名称"是
    // 最有信息量的说明列),再考虑交易对方 / 备注。
    if (_containsAny(s, ['商品名称', '商品说明', '商品描述'])) {
      return 'note';
    }
    if (_containsAny(s, ['备注', '说明', '标题', '摘要', '附言', '交易对方', '商家'])) {
      return 'note';
    }
