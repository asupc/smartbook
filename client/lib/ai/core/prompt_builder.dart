...
3. note: 备注（必须≤15字，超过则精简），提取优先级：
   ...
   - 用户描述（如"给女儿买"）
   - 没有则留空
4. category: 从分类列表选择（转账/还款可填对应分类）
5. type: income、expense 或 transfer（信用卡还款/还贷等"还款"属 transfer，不属消费）
...
11. confidence: 识别置信度 0~1（填 0.9~1.0 表示完全确定；金额/动作明确但时间靠推断填 0.6~0.9；有歧义或仅猜测填 0.2~0.6）
  ...
  /// 短信路径的账单过滤段。
  ///
  /// 短信比截图噪点多(验证码/营销/余额提醒经 native 侧已在入队前剔除,
  /// 这里是 AI 侧的二次兜底),明确要求:不含交易信息(金额+收支动作)时
  /// 返回空数组,并用示例固定行为。
  static const String billGuardForSms = '请先判断以下内容是否为银行/支付服务的交易短信。'
      '以下情况通常不属于交易短信(仅供参考,不仅限于此):\n'
      '- 验证码、密码、安全验证类短信\n'
      '- 营销、活动、优惠类短信\n'
      '- 余额/积分/账单日/还款日等纯提醒短信(没有具体交易金额和收支动作)\n'
      '- 短信是没有金额的\n'
      '\n'
  ...
  ///
  /// 差异:通知文本混杂微信/支付宝的聊天、营销、安全提醒类消息,
  /// 前置过滤侧重"非支付通知一律丢弃"。
  static const String billGuardForNotification = '请先判断以下内容是否为支付/交易信息'
      '(微信/支付宝/银行 App 推送的通知文本)。\n'
      '以下情况通常不属于交易(仅供参考,不仅限于此):\n'
      '- 聊天消息、群聊、公众号文章、朋友圈提醒\n'
      '- 营销、活动、优惠类通知\n'
      '- 验证码、密码、安全验证类通知\n'
      '- 余额/积分/账单日/还款日等纯提醒(没有具体交易金额和收支动作)\n'
      '- 通快递/物流/取件类通知\n'
      '\n'
  ...
  /// 屏幕文本(账单详情页)路径的账单过滤段。
  ///
  /// 文本来自无障碍服务抓取的整棵页面文本树,噪音最多(按钮、导航栏、
  /// 商品页、购物车页都可能带金额)。核心要求:只有「已成交的交易」才输出,
  /// 待支付/浏览中的页面一律视为非账单。
  static const String billGuardForScreen = '请先判断以下内容是否为「已支付/已成交交易」的'
      '账单、订单详情或支付成功页面的文本(支付宝/抖音/京东/微信 App 页面)。\n'
      '以下情况通常不属于交易(仅供参考,不仅限于此):\n'
      '- 商品浏览、商品详情、首页、直播间、购物车(只有价格没有成交结果)\n'
      '- 待支付/待付款/订单确认页(尚未成交,不应记账)\n'
      '- 收件地址、物流、售后、评价等非交易页面\n'
      '- 语音/图片/照片识别页(其他 App 页面通常也不会出现在这里)\n'
      '- 聊天消息、公众号文章、朋友圈\n'
      '\n'
      '注意:信用支付已构成成交 —— 抖音月付/先用后付的订单详情(状态如'
      '「确认收货后付款」、支付方式为「抖音月付」)按普通消费支出记录、'
      'settlement_status 填 settled,不要因「收货后付款」字样返回 pendingOrder。\n'
