...
class _AccountEditPageState extends ConsumerState<AccountEditPage> {
  ...
  // 余额调整(页内直接编辑):统计就绪后预填当前余额
  final TextEditingController _adjustBalanceController = TextEditingController();
  bool _adjusting = false;
  double? _prefilledBalance;
  ...
  Widget build(BuildContext context) {
    ...
    // 余额调整(仅编辑已有账户):日常账户(现金/银行卡/支付/其它,信用卡
    // 与估值账户除外)把账面余额手工对齐实际余额,记一笔排除统计的调整交易。
    // 直接在页内展示当前余额 + 输入框,改完点「确认更新」即入账。
    final accountStatsAsync = isEditing
        ? ref.watch(accountStatsProvider(widget.account!.id))
        : null;
    final canAdjustBalance = isEditing &&
        _selectedType != 'credit_card' &&
        !isValuationOnlyType(_selectedType);
    final currentBalance = accountStatsAsync?.valueOrNull?.balance;
    final balanceReady = canAdjustBalance && currentBalance != null;

    // 统计首次就绪后预填当前余额(仅一次,后续调整/刷新不覆盖用户输入)
    if (isEditing) {
      ref.listen(accountStatsProvider(widget.account!.id), (prev, next) {
        final balance = next.valueOrNull?.balance;
        if (balance != null && _prefilledBalance == null) {
          _prefilledBalance = balance;
          _adjustBalanceController.text = balance.abs().toStringAsFixed(2);
        }
      });
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (isEditing) ...[
                            ...
                            Text(
                              l10n.accountInitialBalanceLocked,
                              style: TextStyle(
                                fontSize: 12,
                    ...
                    SectionCard(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l10n.accountAdjustBalanceLabel,
                                style: _sectionTitle(context)),
                            SizedBox(height: 4.0.scaled(context, ref)),
                            Text(
                              '${l10n.accountBalance}: ${getCurrencySymbol(_selectedCurrency)} ${currentBalance != null ? currentBalance.toStringAsFixed(2) : '…'}',
                              style: TextStyle(
                                fontSize: 13,
                            ...
                            TextFormField(
                              ...
                              enabled: balanceReady && !_adjusting,
                              decoration: filledDec(
                                label: l10n.accountAdjustBalanceNewValue,
                                hint: l10n.accountAdjustBalanceHint,
                                prefix:
                                    '${getCurrencySymbol(_selectedCurrency)} ',
                              ),
                              style: const TextStyle(fontSize: 16),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                // 统计未就绪时禁用(避免点了无响应)
                                onPressed: !balanceReady || _adjusting
                                    ? null
                                    : () =>
                                        _submitAdjustBalance(currentBalance),
                                ...
                                child: _adjusting
                                    ...
                                    : Text(l10n.accountAdjustBalanceUpdate),
                              ),
                            ),
                          ],
                        ),
  ...
  Future<void> _submitAdjustBalance(double currentBalance) async {
    ...
    if (result == null) {
      showToast(context, l10n.accountAdjustBalanceInvalid);
      return;
    }

    final diff = result - currentBalance;
    if (diff.abs() < 0.005) {
      showToast(context, l10n.accountAdjustBalanceSame);
      return;
    }
    ...
    if (!mounted) return;
    // 刷新统计并把输入框回填为新余额,「当前余额」行随 provider 重建更新
    ref.invalidate(accountStatsProvider(account.id));
    _prefilledBalance = result;
    _adjustBalanceController.text = result.abs().toStringAsFixed(2);
    showToast(context, l10n.accountBalanceAdjustedToast);
  }
    ...
    try {
      ...
      if (isEditing) {
        ...
        await repo.updateAccount(
          ...
          // 初始值仅创建时可设:编辑保存不提交该字段(本地 Drift 不更新,
          // 服务端 mutate/push 双路径同样锁定旧值)
