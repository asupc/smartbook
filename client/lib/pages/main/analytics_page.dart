...
class _AnalyticsPageState extends ConsumerState<AnalyticsPage> {
  String _scope = 'month'; // month | year | all | range
  ...
  // M5 自定义区间:仅 _scope == 'range' 时使用(含当天)
  ...
  void _onChartSwipeLeft() {
    if (_scope == 'range') return; // 自定义区间无"下一周期"概念
    final selMonth = ref.read(selectedMonthProvider);
    final now = DateTime.now();
    setState(() {
      if (_scope == 'month') {
  ...
  void _onChartSwipeRight() {
    if (_scope == 'range') return; // 自定义区间无"上一周期"概念
    final selMonth = ref.read(selectedMonthProvider);
    setState(() {
      if (_scope == 'month') {
        // 月视角：切换到上一个月
  ...
  Widget build(BuildContext context) {
    ...
    if (_scope == 'month') {
      ...
      // M5 自定义区间:未选过则回落本月,避免空态
      start = _rangeStart ?? DateTime(now.year, now.month, 1);
      end = (_rangeEnd ?? today).add(const Duration(days: 1));
    } else {
    ...
    if (_type == 'balance') {
      ...
      // 收入或支出模式
      seriesFuture = _scope == 'month' || _scope == 'range'
          ? repo.totalsByDay(
            ...
            bottom: Padding(
              ...
              child: CapsuleSwitcher<String>(
                ...
                options: [
                  ...
                  CapsuleOption(
                    value: 'range',
                    label: AppLocalizations.of(context).analyticsCustom,
                  ),
                  CapsuleOption(
                    value: 'all',
                ...
                return GestureDetector(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      // M5 同比/环比(月/年视角;区间/全部不显示)
                      if (_scope == 'month' || _scope == 'year')
...
String _currentPeriodLabel(
  ...
  switch (scope) {
    ...
    case 'range':
      final s = rangeStart ?? selMonth;
      final e = rangeEnd ?? s;
      String mm(int m) => m.toString().padLeft(2, '0');
      return '${s.year}-${mm(s.month)}-${mm(s.day)} ~ '
          '${e.year}-${mm(e.month)}-${mm(e.day)}';
    case 'all':
...
// ============================================================
// M5 新增:周期对比行(同比/环比)+ 账户分布/Top 商户卡
// ============================================================

/// 单个对比数据(率 + 文案 key + 是否"坏方向"——支出上升/收入下降为坏)。
class _CompareChip {
  final double rate;
  final String label;
  final bool bad;
  const _CompareChip(this.rate, this.label, this.bad);
}

/// M5-A 周期对比行:月视角 环比(较上月)+ 同比(较去年同月);
/// 年视角 环比(较去年)+ 同比(较前年)。仅 支出/收入 模式;
/// 上一周期无数据(<=0)或当前为 0 时不显示。
class _PeriodCompareRow extends ConsumerStatefulWidget {
  final String scope;
  final String type;
  ...
  @override
  ConsumerState<_PeriodCompareRow> createState() => _PeriodCompareRowState();
}

class _PeriodCompareRowState extends ConsumerState<_PeriodCompareRow> {
  Future<List<_CompareChip>?> _fetch() async {
    if (widget.currentTotal == 0) return null;
    final repo = ref.read(repositoryProvider);
    final ledgerId = ref.read(currentLedgerIdProvider);
    ...
    // 上下文标签:月 = 较上月/较去年同月;年 = 较去年/较前年
    final String lastLabel;
    final String yoyLabel;
    final double prevTotal;
    final double yoyTotal;
    if (widget.scope == 'month') {
      // monthlyTotals 返回位置记录 (income, expense)
      final (prevIncome, prevExpense) = await repo.monthlyTotals(
          ledgerId: ledgerId,
          month: DateTime(widget.selMonth.year, widget.selMonth.month - 1, 1));
      ...
      final (prevIncome, prevExpense) = await repo.yearlyTotals(
    ...
    final chips = <_CompareChip>[];
    final prevRate = rate(prevTotal);
    if (prevRate != null) {
      chips.add(_CompareChip(prevRate, lastLabel, isExpense ? prevRate > 0 : prevRate < 0));
    }
    final yoyRate = rate(yoyTotal);
    if (yoyRate != null) {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    return FutureBuilder<List<_CompareChip>?>(
      future: _fetch(),
      builder: (context, snapshot) {
        final chips = snapshot.data;
        if (!snapshot.hasData || chips == null || chips.isEmpty) {
                  ...
                  children: [
                    Icon(
                      c.rate >= 0
                          ...
                          : Icons.arrow_downward,
                      size: 14,
                      color: c.bad ? Colors.orange : Colors.green,
                    ),
                    const SizedBox(width: 4),
                    Text(
                      '${c.label} ${c.rate >= 0 ? '+' : ''}'
                      '${(c.rate * 100).toStringAsFixed(1)}%',
                      style: TextStyle(
                        fontSize: 12,
                        color: c.bad ? Colors.orange : Colors.green,
                      ),
...
/// M5-B 账户分布卡:区间内按账户聚合消费(口径与统计一致),TOP 展示。
class _AccountBreakdownCard extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_AccountBreakdownCard> createState() =>
      _AccountBreakdownCardState();
}
...
class _AccountBreakdownCardState
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<({int? accountId, double total})>>(
      future: _future,
      builder: (context, snapshot) {
        ...
        return Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  l10n.analyticsAccountBreakdown,
                  style: BeeTextTokens.title(context),
                ),
                const SizedBox(height: 8),
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                            item.accountId != null
                                ? (accountName[item.accountId] ?? '#${item.accountId}')
                                : l10n.analyticsNoAccount,
                        ...
                        SizedBox(
                          ...
                          child: LinearProgressIndicator(
                            value: sum == 0 ? 0 : item.total / sum,
                            minHeight: 6,
                            backgroundColor: BeeTokens.textTertiary(context).withValues(alpha: 0.2),
                          ),
                        ),
                      ],
                    ),
...
/// M5-B Top 商户卡:区间内按备注(商户)聚合消费,TOP 10。
class _MerchantTopCard extends ConsumerStatefulWidget {
  final BaseRepository repo;
  final int ledgerId;
  final DateTime start;
  final DateTime end;
  ...
  @override
  ConsumerState<_MerchantTopCard> createState() => _MerchantTopCardState();
}
