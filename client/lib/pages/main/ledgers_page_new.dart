...
class _LedgersPageNewState extends ConsumerState<LedgersPageNew> {
  ...
  Widget _buildSplitLedgerList(
    ...
    // 共享账本是 SmartBook Cloud 独有能力(server 端的成员管理 / WS fan-out
     // 都在 SmartBook Cloud 后端),非 SmartBook Cloud 用户(local / WebDAV /
     // S3 / Supabase 等)就算扫码也走不通,按钮藏起来避免误导。
    final cloudConfigAsync = ref.watch(activeCloudConfigProvider);
    final isSmartBookCloud =
        cloudConfigAsync.valueOrNull?.type == CloudBackendType.smartbookCloud;
  ...
  Future<void> _handleRemoteLedgerTap(BuildContext context, LedgerDisplayItem ledger) async {
    ...
    try {
      ...
      if (syncService is SyncEngine) {
        // SmartBook Cloud 路径（sync_changes 增量日志模型）：
        ...
        //
        // 不走 `_fullPull`（整包 JSON 下载）—— 那是 S3/WebDAV 的玩法，SmartBook
  ...
  Future<void> _showLocalLedgerActions(BuildContext context, LedgerDisplayItem ledger) async {
    ...
    // 共享账本/成员管理是 SmartBook Cloud 独有能力,非 SmartBook Cloud 模式
    // (local / WebDAV / S3 / Supabase 等)直接隐藏这些入口。
    final cloudConfig = ref.read(activeCloudConfigProvider).valueOrNull;
    final isSmartBookCloud =
        cloudConfig?.type == CloudBackendType.smartbookCloud;
      ...
      builder: (dctx) {
        ...
        return SimpleDialog(
          ...
          children: [
            ...
            // Editor 可看列表 + 退出账本)。非 SmartBook Cloud 模式没成员概念,
            // 整个入口隐藏。
            if (isSmartBookCloud) ...[
              SimpleDialogOption(
  ...
  Future<void> _handleBatchRestore(BuildContext context) async {
    ...
    try {
      ...
      int failed = 0;
      if (syncService is SyncEngine) {
        // SmartBook Cloud 批量（sync_changes 日志模型）：
