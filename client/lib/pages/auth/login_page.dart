...
class _AuthPageState extends ConsumerState<AuthPage> {
  ...
  Future<void> _loadSavedCredentials() async {
    try {
      ...
      if (cloudConfig.type == CloudBackendType.smartbookCloud) {
        // SmartBook Cloud：勾选"记住账号"时同时存邮箱+密码，
        // 作为 token 失效时的兜底登录途径（见 smartbookCloudProviderInstance
        // 里的 fallback signInWithEmail）。
        savedEmail = cloudConfig.smartbookCloudEmail;
        savedPassword = cloudConfig.smartbookCloudPassword;
      } else {
        return;
      }
  ...
  Future<void> _saveCredentials(String email, String password) async {
    try {
      ...
      if (cloudConfig.type == CloudBackendType.smartbookCloud) {
        // SmartBook Cloud：勾选"记住账号"时存邮箱+密码 —— token 机制平时够用，
        // 但 token 失效 / 老版本升级 / 本地 SharedPreferences 被清等场景都靠
        // 这份密码做兜底自动登录。
        final updatedConfig = CloudServiceConfig(
          type: cloudConfig.type,
          name: cloudConfig.name,
          smartbookCloudBaseUrl: cloudConfig.smartbookCloudBaseUrl,
          smartbookCloudApiPrefix: cloudConfig.smartbookCloudApiPrefix,
          smartbookCloudEmail: _rememberAccount ? email : null,
          smartbookCloudPassword: _rememberAccount ? password : null,
        );
        await store.saveOnly(updatedConfig);
        ref.invalidate(smartbookCloudConfigProvider);
        ref.invalidate(activeCloudConfigProvider);
        logger.info('auth',
            'SmartBook Cloud 账号密码保存状态：${_rememberAccount ? "已保存" : "已清除"}');
      }
    } catch (e, st) {
  ...
  /// 按当前云后端选注册指引文档的 topic:Supabase / SmartBook Cloud 各跳自己的
