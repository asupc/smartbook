...
class _DevicesPageState extends ConsumerState<DevicesPage> {
  ...
  List<SmartBookCloudDevice> _devices = const [];
  List<SmartBookCloudDevice> _allSessions = const [];
  ...
  int _recentScore(SmartBookCloudDevice device) {
    return device.lastSeenAt?.millisecondsSinceEpoch ??
        device.createdAt?.millisecondsSinceEpoch ??
        0;
  }

  String _fingerprint(SmartBookCloudDevice device) {
  ...
  List<SmartBookCloudDevice> _sorted(List<SmartBookCloudDevice> devices) {
    final out = devices.toList(growable: false);
    out.sort((a, b) => _recentScore(b).compareTo(_recentScore(a)));
    return out;
  }

  List<String> _targetSessionIds(SmartBookCloudDevice device) {
    if (_showAllSessions) {
      return [device.id];
    }
  ...
  /// 获取 SmartBookCloudProvider 实例（仅 smartbookCloud 后端可用）
  Future<SmartBookCloudProvider> _getCloudProvider() async {
    ...
    if (services.provider == null || services.provider is! SmartBookCloudProvider) {
      throw StateError(
          AppLocalizations.of(context).cloudCollabUnavailableMessage);
    }
    return services.provider as SmartBookCloudProvider;
  }
  ...
  Future<void> _revokeDevice(SmartBookCloudDevice device) async {
