          ...
          Expanded(
            child: authAsync.when(
              ...
              data: (auth) => FutureBuilder<CloudUser?>(
                ...
                final isSmartBookCloud = cloudConfig.hasValue &&
                    cloudConfig.value!.type == CloudBackendType.smartbookCloud;
                final needsLogin = cloudConfig.hasValue &&
                    (cloudConfig.value!.type == CloudBackendType.supabase ||
                     cloudConfig.value!.type == CloudBackendType.smartbookCloud);
                // Supabase 和 SmartBook Cloud 需要登录，其他云服务（iCloud/S3/WebDAV）使用配置文件认证
                    ...
                    SectionCard(
                      ...
                      child: Column(
                        children: [
                          ...
                          // ===== SmartBook Cloud 模式：同步状态 + 登录（无需手动操作） =====
                          if (isSmartBookCloud) ...[
                            // 登录（未登录时显示登录入口）
                                    ...
                                    AppListTile(
                                      ...
                                      onTap: () async {
                                        final providerAsync = ref
                                            .read(smartbookCloudProviderInstance);
