...
import '../../services/export/json_backup_service.dart';
...
class _ExportPageState extends ConsumerState<ExportPage> {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // M5 JSON 结构化导出(备份):账本+账户+分类+交易(含标签/附件名)
                  const SizedBox(height: 12),
                  OutlinedButton.icon(
                    onPressed: exporting ? null : () => _exportJson(repo, ledgerId),
                    icon: const Icon(Icons.data_object),
                    label: Text(AppLocalizations.of(context).exportJsonButton),
                  ),
  ...
  Future<void> _export(BaseRepository repo, int ledgerId) async {
    try {
      ...
      if (Platform.isIOS) {
        ...
        // Android: 直接保存到公共 Download/SmartBook 目录
        const downloadPath = '/storage/emulated/0/Download/SmartBook';
      ...
      final path = p.join(directory, 'smartbook_$ts.csv');
  ...
  /// M5 JSON 结构化导出(备份):自包含(账本/账户/分类/交易/标签/附件名),
  /// 结构可回读;不含截图原文与周期记账/预算(后者走配置 YAML 导出)。
  Future<void> _exportJson(BaseRepository repo, int ledgerId) async {
    try {
      setState(() {
        exporting = true;
        progress = 0;
      ...
      final txs = transactionsWithCategory.map((txw) => txw.t).toList();
      final ids = txs.map((t) => t.id).toList();
      final tagsMap = await repo.getTagsForTransactions(ids);
      final attachmentsMap = await repo.getAttachmentsForTransactions(ids);

      final backupTxs = txs.map((t) {
        return BackupTx(
          id: t.id,
          type: t.type,
          amount: t.amount,
          ...
          tags: (tagsMap[t.id] ?? []).map((tag) => tag.name).toList(),
          attachments:
              (attachmentsMap[t.id] ?? []).map((a) => a.fileName).toList(),
        );
      }).toList();
      ...
      final categories =
          <({int id, String name, String kind, int? parentId, int level})>[];
      for (final kind in ['expense', 'income']) {
        for (final top in await repo.getTopLevelCategories(kind)) {
          categories.add((
            id: top.id,
            name: top.name,
            kind: top.kind,
          ...
          for (final sub in await repo.getSubCategories(top.id)) {
            categories.add((
              id: sub.id,
              name: sub.name,
              kind: sub.kind,
      ...
      final ts = DateFormat('yyyyMMdd_HHmmss').format(DateTime.now());
      final path = p.join(directory, 'smartbook_backup_$ts.json');
      const encoder = JsonEncoder.withIndent('  ');
      await File(path)
          .writeAsString(encoder.convert(json), encoding: utf8);
