...
import '../providers/smartbook_cloud_provider.dart';
...
Future<({CloudProvider? provider, CloudAuthService? auth})> createCloudServices(
  ...
  switch (config.type) {
    ...
    case CloudBackendType.smartbookCloud:
      final provider = SmartBookCloudProvider();
      await provider.initialize({
        'baseUrl': config.smartbookCloudBaseUrl!,
        'apiPrefix': config.smartbookCloudApiPrefix ?? '/api/v1',
      });
