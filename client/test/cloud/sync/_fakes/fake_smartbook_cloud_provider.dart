// FakeSmartBookCloudProvider — SyncEngine e2e 测试的 in-memory 替身。
//
// 设计:
//   - extends SmartBookCloudProvider 真类(默认构造无副作用,_auth/_storage
//     在 initialize() 调用后才被设)
...
// =====================================================================
// FakeSmartBookCloudAuthService — extends 真类,覆盖 currentUserId/currentDeviceId
// =====================================================================

class FakeSmartBookCloudAuthService extends SmartBookCloudAuthService {
  FakeSmartBookCloudAuthService({
  ...
  // 覆盖 SmartBookCloudAuthService 自身的 getter(不在 CloudAuthService 接口
...
// =====================================================================
// FakeSmartBookCloudStorageService — 内存模拟 storage(用于 fullPush JSON 等)
// =====================================================================

class FakeSmartBookCloudStorageService implements CloudStorageService {
...
// =====================================================================
// FakeSmartBookCloudProvider — 主入口
// =====================================================================

class FakeSmartBookCloudProvider extends SmartBookCloudProvider {
  FakeSmartBookCloudProvider({
    String? userId = 'test-user-id',
    String? deviceId = 'test-device-id',
  }) {
    _fakeAuth = FakeSmartBookCloudAuthService(
      userId: userId,
      deviceId: deviceId,
    );
    _fakeStorage = FakeSmartBookCloudStorageService();
  }

  late final FakeSmartBookCloudAuthService _fakeAuth;
  late final FakeSmartBookCloudStorageService _fakeStorage;

  /// In-memory server 状态:全部 sync_changes 流。
  /// 测试通过 [pushFakeChange] 往里塞;[pullChanges] 按 since 切片返回。
  final List<SmartBookCloudSyncChange> _serverChanges = [];

  /// 在线 ledger list(server 端 `/sync/ledgers` 返回)。
  /// 测试通过 [pushFakeLedger] 注入。
  final List<SmartBookCloudReadLedger> _serverLedgers = [];
  ...
  // ====== 覆盖 SmartBookCloudProvider getter ======
  ...
  SmartBookCloudSyncChange pushFakeChange({
    ...
    final change = SmartBookCloudSyncChange(
  ...
  void pushFakeLedger({
    ...
    _serverLedgers.add(SmartBookCloudReadLedger(
      ledgerId: ledgerId,
  ...
  /// 模拟 server 推 WS 事件
  void emitRealtimeEvent(SmartBookCloudRealtimeEvent event) {
  ...
  Future<SmartBookCloudWriteCommitMeta> writeCreateLedger({
    ...
    final meta = SmartBookCloudWriteCommitMeta(
...
class _StorageProxy implements CloudStorageService {
  ...
  final FakeSmartBookCloudStorageService _real;
