// SyncEngine 端到端测试。
//
// 用 in-memory Drift + FakeSmartBookCloudProvider 跑完整 pull/push/apply
...
import 'package:smartbook/cloud/sync_service.dart' show SyncDiff;
...
void main() {
  ...
  group('SyncEvent stream(PR 1 解耦改造)', () {
    test('WS pull 完成 emit PullCompleted 到 events stream', () async {
      ...
      provider.emitRealtimeEvent(SmartBookCloudRealtimeEvent(
        type: 'sync_change',
        ledgerId: 'L1',
        rawData: const {},
      ));
