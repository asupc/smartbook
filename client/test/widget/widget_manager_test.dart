...
import 'package:smartbook/widget/widget_manager.dart';
import 'package:smartbook/widget/widget_spec.dart';
  ...
  group('matchInstalledSpecs', () {
    ...
    test('Android 多尺寸类型:一个 provider 类名返回该类型全部尺寸 spec', () {
      ...
      final infos = [
        HomeWidgetInfo(
          androidClassName:
              'com.smartbook.zhi.SmartBookNetWorthWidgetProvider',
          androidWidgetId: 7,
        ),
      ];
    ...
    test('glance 小号补全:iOS 同 kind 的 systemSmall family 命中 glanceSmall,'
      ...
      expect(
        matchInstalledSpecs([
          HomeWidgetInfo(iOSKind: 'SmartBookWidget', iOSFamily: 'systemSmall'),
        ]),
      ...
      expect(
        matchInstalledSpecs([
          HomeWidgetInfo(iOSKind: 'SmartBookWidget', iOSFamily: 'systemMedium'),
        ]),
        [WidgetSpec.glanceMedium],
      );
    });
