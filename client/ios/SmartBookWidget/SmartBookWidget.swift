...
struct SmartBookEntry: TimelineEntry {
    let date: Date
    let widgetImagePath: String
}

struct SmartBookProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SmartBookEntry {
        SmartBookEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (SmartBookEntry) -> ()) {
        ...
        let entry = SmartBookEntry(date: Date(), widgetImagePath: imagePath)
        completion(entry)
    }
...
struct SmartBookWidgetEntryView : View {
    var entry: SmartBookProvider.Entry
    @Environment(\.widgetFamily) var widgetFamily

    private let expenseURL = URL(string: "smartbook://new?type=expense")!
    private let incomeURL = URL(string: "smartbook://new?type=income")!
...
struct SmartBookWidget: Widget {
    let kind: String = "SmartBookWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SmartBookProvider()) { entry in
            if #available(iOS 17.0, *) {
                SmartBookWidgetEntryView(entry: entry)
