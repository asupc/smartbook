...
struct SmartBookBudgetEntry: TimelineEntry {
    let date: Date
    let widgetImagePath: String
}

struct SmartBookBudgetProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SmartBookBudgetEntry {
        SmartBookBudgetEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (SmartBookBudgetEntry) -> ()) {
        // 添加页预览(isPreview):运行时图片在预览上下文读不到(App
        // Group 访问受限,添加页只会显示占位色块),改用 bundle 内静态
        // 预览资产,详见 WidgetPreviewAssets。
        if context.isPreview {
            completion(SmartBookBudgetEntry(
                date: Date(),
                widgetImagePath: WidgetPreviewAssets.path(forImageKey: imageKey(for: context.family))))
            return
        }
        ...
        let entry = SmartBookBudgetEntry(date: Date(), widgetImagePath: imagePath)
        completion(entry)
    }
...
struct SmartBookBudgetWidgetEntryView : View {
    var entry: SmartBookBudgetProvider.Entry
    @Environment(\.widgetFamily) var widgetFamily

    // 预算卡片点击 → 预算页。第一版整块点击、不分区。
    // TODO: 中号有分类用量列表时，考虑按行分区深链到该分类的筛选明细。
    private let budgetURL = URL(string: "smartbook://open?page=budget")!
...
struct SmartBookBudgetWidget: Widget {
    let kind: String = "SmartBookBudgetWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SmartBookBudgetProvider()) { entry in
            if #available(iOS 17.0, *) {
                SmartBookBudgetWidgetEntryView(entry: entry)
