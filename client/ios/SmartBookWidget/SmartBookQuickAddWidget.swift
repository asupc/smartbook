...
struct SmartBookQuickAddEntry: TimelineEntry {
    let date: Date
    let widgetImagePath: String
}

struct SmartBookQuickAddProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SmartBookQuickAddEntry {
        SmartBookQuickAddEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (SmartBookQuickAddEntry) -> ()) {
        // 添加页预览(isPreview):运行时图片在预览上下文读不到(App
        // Group 访问受限,添加页只会显示占位色块),改用 bundle 内静态
        // 预览资产,详见 WidgetPreviewAssets。
        if context.isPreview {
            completion(SmartBookQuickAddEntry(
                date: Date(),
                widgetImagePath: WidgetPreviewAssets.path(forImageKey: imageKey(for: context.family))))
            return
        }
        ...
        let entry = SmartBookQuickAddEntry(date: Date(), widgetImagePath: imagePath)
        completion(entry)
    }
...
struct SmartBookQuickAddWidgetEntryView : View {
    var entry: SmartBookQuickAddProvider.Entry
    @Environment(\.widgetFamily) var widgetFamily

    // 快速记账卡片点击 → 新建支出。第一版整块点击、不分区。
    // TODO: 常用分类格拆分点击区域后，改为按分类深链
    // `smartbook://new?type=expense&category=<id>`（分类 id 取自渲染时
    // 的取数结果，需要额外把每格坐标信息一并写入共享存储或改用
    // AppIntent 交互式布局）。
    private let addExpenseURL = URL(string: "smartbook://new?type=expense")!
...
struct SmartBookQuickAddWidget: Widget {
    let kind: String = "SmartBookQuickAddWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SmartBookQuickAddProvider()) { entry in
            if #available(iOS 17.0, *) {
                SmartBookQuickAddWidgetEntryView(entry: entry)
