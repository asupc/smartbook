...
struct SmartBookNetWorthEntry: TimelineEntry {
    let date: Date
    let widgetImagePath: String
}

struct SmartBookNetWorthProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SmartBookNetWorthEntry {
        SmartBookNetWorthEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (SmartBookNetWorthEntry) -> ()) {
        // 添加页预览(isPreview):运行时图片在预览上下文读不到(App
        // Group 访问受限,添加页只会显示占位色块),改用 bundle 内静态
        // 预览资产,详见 WidgetPreviewAssets。
        if context.isPreview {
            completion(SmartBookNetWorthEntry(
                date: Date(),
                widgetImagePath: WidgetPreviewAssets.path(forImageKey: imageKey(for: context.family))))
            return
        }
        ...
        let entry = SmartBookNetWorthEntry(date: Date(), widgetImagePath: imagePath)
        completion(entry)
    }
...
struct SmartBookNetWorthWidgetEntryView : View {
    var entry: SmartBookNetWorthProvider.Entry
    @Environment(\.widgetFamily) var widgetFamily

    // 净资产卡片点击 → 资产页。第一版整块点击、不分区；后续如需按账户明细
    // 列表分区跳转，可参考 SmartBookWidget.swift 的 GeometryReader 分区写法。
    // TODO: 大号有账户明细列表时，考虑按行分区深链到具体账户。
    private let assetsURL = URL(string: "smartbook://open?page=assets")!
...
struct SmartBookNetWorthWidget: Widget {
    let kind: String = "SmartBookNetWorthWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SmartBookNetWorthProvider()) { entry in
            if #available(iOS 17.0, *) {
                SmartBookNetWorthWidgetEntryView(entry: entry)
