# SmartBook Cloud &nbsp; [English](./README.en.md)
...
**[SmartBook(智记)](https://github.com/TNT-Likely/BeeCount) App 的自部署同步云端。** 让 iOS / Android / Web 三端共用一份完全属于你的账本 — 无广告、无订阅、无第三方依赖。

![SmartBook Cloud Web 控制台](./docs/screenshot-zh.png)

> 🤖 **新:用 LLM 直接管理账本** — SmartBook Cloud 内置 [MCP](https://count.beejz.com/docs/mcp) server,在 Claude Desktop / Cursor / Cline 里跟 LLM 自然语言对话就能查询交易、记账、改预算。👉 **[查看 MCP 文档](https://count.beejz.com/docs/mcp)**

---

## 🤔 为什么选 SmartBook Cloud

### vs 其他自托管账本

| 项 | SmartBook Cloud | Firefly III | Actual Budget | Maybe Finance |
|---|---|---|---|---|
...
- **AES-256 加密 zip** — 备份文件是标准的密码加密 zip,**用户从对象存储下载下来双击 → macOS Archive Utility / 7-Zip / Keka / WinRAR 自动弹密码框 → 解开**(脱离 SmartBook 服务也能恢复)
...
只要你有 .zip 文件 + 口令,任何系统、任何标准解压工具都能解开备份,不依赖 SmartBook 服务存在。
...
docker compose up -d
# 查看首次启动生成的随机管理员账号密码:
docker compose logs smartbook-cloud | grep -A 10 "初次启动"
```

看到类似:

```
 SmartBook Cloud — 初次启动,已自动创建管理员账号:
...
- App 里选「SmartBook Cloud」,填服务器地址 + 上面账号登录
...
安装 [SmartBook](https://github.com/TNT-Likely/BeeCount) App(iOS / Android),然后在 App 中:

1. 设置 → 云服务 → SmartBook Cloud
...
- 把 SmartBook Cloud 作为商业 SaaS 提供给客户
- 在盈利性组织中部署使用
- 基于本软件提供付费云服务
- 转售或集成到付费产品里
