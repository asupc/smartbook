...
export async function fetchDuplicateTransactions(
  token: string,
): Promise<DuplicateGroup[]> {
  return authedGet<DuplicateGroup[]>('/admin/duplicate-transactions', token)
}

export async function executeDuplicateCleanup(
  token: string,
  items: DuplicateDeleteItem[],
): Promise<DuplicateCleanResult> {
  return authedPost<DuplicateCleanResult>(
    '/admin/duplicate-transactions/clean',
    token,
    { items },
  )
}
