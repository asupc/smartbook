...
// 构建期注入的 API 前缀,只接受 / 相对前缀(默认 /api/v1)或 http(s):// 完整
// URL。Git Bash 的 MSYS 路径转换曾把 /api/v1 污染成 C:/Program Files/Git/api/v1
// 编译进 bundle,导致登录请求打到 file:///C:/... 本地地址 —— 这里兜底回退,
// 不再让坏值溜进请求 URL。
function safeApiBase(): string {
  const raw = ((import.meta as any).env?.VITE_API_BASE_URL || '/api/v1').trim()
  if (!/^(https?:\/\/|\/)/.test(raw)) return '/api/v1'
  if (/%20|file:|[\\]/.test(raw)) return '/api/v1'
  return raw
}

export const API_BASE = safeApiBase()
...
/** 带鉴权下载二进制(AI 日志输入图片等)。非 2xx 时按 ApiError 抛出(文本
 * 响应,调用方自行 catch)。 */
export async function authedGetBlob(path: string, token: string): Promise<Blob> {
  const res = await authedFetch(
    (tok) => fetch(`${API_BASE}${path}`, { headers: authHeaders(tok), cache: 'no-store' }),
    token
  )
  if (!res.ok) {
    throw await extractApiError(res)
