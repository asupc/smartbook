...
import { Card, CardContent, CardHeader, CardTitle } from '@smartbook/ui'
...
export function ListTableShell({ title, actions, children }: ListTableShellProps) {
  return (
    <Card className="bc-table-panel overflow-hidden">
      {/* mobile: 标题独占一行,action 换行跟上,避免标题被挤成竖向一字排;
          sm+: 横排 + justify-between 回到原样式。 */}
      <CardHeader className="flex flex-col gap-3 border-b border-border/60 bg-muted/20 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
