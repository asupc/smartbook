        ...
        {mode === 'text' && (
          <div className="border-b border-border/40 p-3">
            <Input.TextArea
              ...
              style={{ minHeight: 80, maxHeight: 200, resize: 'vertical', fontSize: 12 }}
              readOnly={status === 'parsing'}
            />
            {(status === 'idle' || status === 'parsed') && (
              <div className="mt-2 flex justify-end">
                <Button size="small" onClick={runParse} disabled={!text.trim()} icon={<Send className="h-3 w-3" />}>
                  {status === 'parsed'
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          ...
          {status === 'error' && errorMsg && (
            <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              ...
              <Button variant="outlined" size="small" className="mt-2" icon={<RotateCw className="h-3 w-3" />} onClick={runParse}>
