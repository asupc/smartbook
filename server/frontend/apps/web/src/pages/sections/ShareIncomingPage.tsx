...
const SHARE_CACHE = 'smartbook-share-target-v1'
const CSV_RE = /\.(csv|tsv|xlsx?)$/i
const IMAGE_RE = /^image\//

/**
 * Share Target / File Handler 的统一着陆页 —— 用户从系统分享菜单选「智记」
...
export function ShareIncomingPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      //    本页前会把 File[] 暂存到 window.__smartbookPendingFiles,这里取出。
      try {
        const pending = (window as unknown as { __smartbookPendingFiles?: File[] })
          .__smartbookPendingFiles
        if (Array.isArray(pending) && pending.length > 0) {
          for (const f of pending) files.push(f)
          ;(window as unknown as { __smartbookPendingFiles?: File[] }).__smartbookPendingFiles = []
        }
      } catch {
        // ignore
      }
