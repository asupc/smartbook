  ...
  return (
    <div className="space-y-4">
      <Card
        ...
        extra={
          <Button size="small" icon={<Plus className="h-4 w-4" />} onClick={() => setCreateOpen(true)}>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label htmlFor="pat-name" style={fieldLabelStyle}>{t('settings.pats.create.fields.name')}</label>
            <Input
              id="pat-name"
              value={newName}
          ...
          <div className="space-y-1.5">
            <label style={fieldLabelStyle}>{t('settings.pats.create.fields.scope')}</label>
            <Select
              value={newScope}
              onChange={(v) => setNewScope(v as 'read' | 'write')}
              options={[
                { value: 'read', label: t('settings.pats.scope.readOnly') },
                { value: 'write', label: t('settings.pats.scope.readWrite') },
              ]}
              style={{ width: '100%' }}
            />
            <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-1.5">
            <label style={fieldLabelStyle}>{t('settings.pats.create.fields.expiration')}</label>
            <Select
              value={newExpiration}
              onChange={(v) => setNewExpiration(v as ExpirationOption)}
      ...
      >
        {createdToken ? (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">{t('settings.pats.created.description')}</p>
            <div className="rounded-md border bg-muted/50 p-3 font-mono text-sm break-all">
              {createdToken.token}
            </div>
            <Button block variant="filled" icon={<Copy className="h-4 w-4" />} onClick={() => void handleCopy()}>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label htmlFor="pat-edit-name" style={fieldLabelStyle}>{t('settings.pats.create.fields.name')}</label>
            <Input
              id="pat-edit-name"
              value={editName}
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={editScope}
              onChange={(v) => setEditScope(v as 'read' | 'write')}
              options={[
                { value: 'read', label: t('settings.pats.scope.readOnly') },
                { value: 'write', label: t('settings.pats.scope.readWrite') },
              ]}
  ...
  return (
    <Card
      ...
      extra={
        <Select
          value={statusFilter}
          onChange={(v) => { setStatusFilter(v as HistoryStatusFilter); setPage(0) }}
          options={[
            { value: 'all', label: t('settings.mcpCalls.filter.all') },
            { value: 'ok', label: t('settings.mcpCalls.filter.ok') },
            { value: 'error', label: t('settings.mcpCalls.filter.error') },
          ]}
          style={{ width: 140 }}
        />
      }
          ...
          {total > HISTORY_PAGE_SIZE ? (
            <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                <Button type="text" size="small" disabled={page === 0} onClick={() => setPage((p) => Math.max(0, p - 1))}>
                  {t('common.previous')}
                </Button>
                <span>
                  {page + 1} / {totalPages}
                </span>
                <Button type="text" size="small" disabled={page + 1 >= totalPages} onClick={() => setPage((p) => p + 1)}>
                  {t('common.next')}
                </Button>
              </div>
            </div>
