...
const fieldLabelStyle: React.CSSProperties = { fontWeight: 500, marginBottom: 4, display: 'block' }
        ...
        <div className="grid w-full gap-8 lg:grid-cols-[1fr_minmax(0,420px)] lg:items-center">
          ...
          <div className="hidden space-y-6 lg:block">
            <div className="flex items-center gap-3">
              <img src="/branding/smartbook_icon.svg" alt={t('shell.appName')} className="h-12 w-12" />
              <div>
          ...
          <div className="w-full">
            <div className="rounded-2xl border border-border/60 bg-card/90 p-8 shadow-xl backdrop-blur-md">
              <div className="mb-6 flex items-center gap-3 lg:hidden">
                <img src="/branding/smartbook_icon.svg" alt="" className="h-10 w-10" />
                <div>
              ...
              <form className="space-y-4" onSubmit={onSubmit}>
                <div className="space-y-1.5">
                  <label style={fieldLabelStyle} htmlFor="login-email">{t('login.email')}</label>
                  <Input
                    id="login-email"
                    autoComplete="email"
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    <label style={fieldLabelStyle} htmlFor="login-password">{t('login.password')}</label>
                ...
                {/* 初次部署的用户经常不知道初始密码哪里来 — 自部署 server 在
                    ...
                    下次不再显示。 */}
                <InitialPasswordHint />
                <Button className="w-full" htmlType="submit" disabled={loading} loading={loading}>
                  {t('login.submit')}
                </Button>
              </form>
              ...
              {notice && (
                <Alert
                  className="mt-4"
                  type={notice.type === 'destructive' ? 'error' : 'info'}
                  message={
                    <div>
                      <p className="mb-1 font-medium">{notice.title}</p>
                      <p>{notice.message}</p>
                    </div>
                  }
                />
 ...
 * 在登录态,× 关闭后写 localStorage,下次访问不再展示。即便关掉,密码 label
 * 旁的 (?) 图标 hover 仍能看见同款文案,信息不会彻底丢失。
 */
