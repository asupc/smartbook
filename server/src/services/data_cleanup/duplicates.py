...
同一账本内,「交易类型 + 金额 + 交易时间(同一分钟)」相同的多笔交易视为重复
(客户端自动记账最常见的重复来源:同一笔账单被屏幕 OCR 和图片 OCR 各记一次,
或截图 / 短信 / 通知多渠道重复入账)。
...
def scan_duplicate_transactions(db: Session) -> list[DuplicateGroup]:
    """扫描全库,把重复交易按 (ledger_id, tx_type, 金额, 分钟) 分组返回。
    ...
    for (ledger_id, tx_type, amount, happened_at), members in groups.items():
        if len(members) < 2:
            continue
