...
def pull_changes(
    ...
    metrics.inc("smartbook_sync_pull_requests_total")
    ...
    if device_id:
        ...
        if not device:
            metrics.inc("smartbook_sync_pull_failed_total")
    ...
    if device_id:
        # 核心契约:过滤"自己 push 的变更"(本地已存在,避免拉回)。
        # 注意 SQL 三值逻辑:`NULL != 'x'` 结果为 NULL(视为假),若直接写
        # `updated_by_device_id != device_id`,旧版本(不写该字段)的行会被
        # 连同本设备的行一起静默过滤,mobile 永远拉不到历史数据。
        # 必须显式放行 NULL 行 —— 客户端 applyRemoteChange 自己还会按
        # updated_by_device_id == 本机 device 再跳一次,双保险。
        query = query.where(
            or_(
