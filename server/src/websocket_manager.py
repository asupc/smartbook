...
class WSConnectionManager:
    ...
    async def connect(self, user_id: str, websocket: WebSocket) -> None:
        ...
        metrics.set_gauge("smartbook_online_ws_users", float(len(self._connections)))
...
async def broadcast_to_ledger(
    *,
    ...
    ``ledger_id`` 必须是 ``Ledger.id`` (String(36) UUID 主键),不是
    ``external_id`` — LedgerMember.ledger_id 是同型 str 外键。传 external_id
    会让查询匹配不到任何成员,fan-out 静默失败。
