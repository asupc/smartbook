...
def update_account(snapshot: dict, account_id: str, payload: dict) -> dict:
    ...
    # 初始值仅在创建时可设(create_account);更新请求里的 initial_balance
    # 一律忽略 —— 余额变更走「调整余额」(记一笔 exclude_from_stats 的调整
    # 交易,保留审计),App / Web 跨端一致。
    _apply_account_optional_fields(account, payload)
...
def delete_account(snapshot: dict, account_id: str, payload: dict | None = None) -> dict:
    ...
    #
    # 关联判定必须用 **accountId(syncId) 精确匹配**,不能用 old_name 名字匹配:
    # 同名账户时会把"另一个同名账户的交易"误判成本账户的关联交易,导致删不掉
    # (报 "account has linked transactions")。仅当交易**完全没有** accountId
    # 字段(老数据)时才回退名字匹配 —— 名字是冗余字段,同名时不可靠。
    normalized_account_id = str(account.get("syncId") or "").strip()
    linked = 0
    for tx in _ensure_list(target, "items"):
        has_syncid_fields = any(
            k in tx for k in ("accountId", "fromAccountId", "toAccountId")
        )
        if has_syncid_fields:
            # 现代数据:按 syncId 精确匹配,命中才计数。
            if (
                tx.get("accountId") == normalized_account_id
                or tx.get("fromAccountId") == normalized_account_id
                or tx.get("toAccountId") == normalized_account_id
            ):
                linked += 1
            continue
        # 老数据缺 accountId 字段:回退名字匹配。
        if old_name and (
    ...
    if normalized_account_id:
        for tx in _ensure_list(target, "items"):
            # 有 syncId 字段的交易只按 syncId 剥离,不碰名字(避免同名账户误抹)。
            if tx.get("accountId") == normalized_account_id:
                tx.pop("accountId", None)
                tx.pop("accountName", None)
            if tx.get("fromAccountId") == normalized_account_id:
                tx.pop("fromAccountId", None)
                tx.pop("fromAccountName", None)
            if tx.get("toAccountId") == normalized_account_id:
                tx.pop("toAccountId", None)
                tx.pop("toAccountName", None)
    elif old_name:
        # 老数据(交易无 accountId 字段)才回退名字剥离。
        for tx in _ensure_list(target, "items"):
            if tx.get("accountName") == old_name:
                tx.pop("accountName", None)
            if tx.get("fromAccountName") == old_name:
