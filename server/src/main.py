...
from .routers import admin_backup, mcp_calls
...
app = FastAPI(
    title=_smartbook_cloud_name,
    version=_smartbook_cloud_version,
    description="SmartBook 智记 v1 API",
)
...
def public_version() -> dict:
    return {"name": _smartbook_cloud_name, "version": _smartbook_cloud_version}
...
@app.on_event("startup")
async def _start_log_retention() -> None:  # noqa: B008
    import asyncio
    ...
    async def _loop() -> None:
        while True:
            ...
            try:
                await asyncio.to_thread(_prune_retention_logs)
            except Exception:
                logging.getLogger(__name__).exception("call log retention failed")

    app.state.log_retention_task = asyncio.create_task(_loop())
...
@app.on_event("shutdown")
async def _stop_log_retention() -> None:  # noqa: B008
