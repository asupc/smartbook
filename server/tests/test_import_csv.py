...
from src.services.import_data.parsers.smartbook import SmartBookParser
...
def test_smartbook_parser_sniff_and_mapping():
    csv = (
    ...
    assert data.source_format == "smartbook"
...
def test_upload_and_preview_happy_path():
    ...
    try:
        ...
        files = {"file": ("test.csv", _smartbook_csv().encode("utf-8"), "text/csv")}
        r = client.post(
        ...
        body = r.json()
        assert body["source_format"] == "smartbook"
...
def test_second_upload_evicts_first():
    ...
    try:
        ...
        r = client.post(
            "/api/v1/import/upload",
            files={"file": ("a.csv", _smartbook_csv().encode("utf-8"), "text/csv")},
            data={"target_ledger_id": ledger_id},
            headers={"Authorization": f"Bearer {token}"},
        )
        ...
        r2 = client.post(
            "/api/v1/import/upload",
            files={"file": ("b.csv", _smartbook_csv().encode("utf-8"), "text/csv")},
            data={"target_ledger_id": ledger_id},
            headers={"Authorization": f"Bearer {token}"},
        )
...
def test_execute_creates_transactions_and_atomic_rollback_on_error(monkeypatch):
    ...
    try:
        ...
        r = client.post(
            "/api/v1/import/upload",
            files={"file": ("ok.csv", _smartbook_csv().encode("utf-8"), "text/csv")},
            data={"target_ledger_id": ledger_id},
            headers={"Authorization": f"Bearer {token}"},
        )
