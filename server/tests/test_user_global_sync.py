...
def test_pull_null_updated_by_device_id_not_filtered():
    """带 device_id 的 /sync/pull 必须返回 updated_by_device_id IS NULL 的历史行。

    背景:2026-09 线上发现"服务端有数据、App 看不到"(交易/账户/分类/预算
    全类型)。根因是 pull 里直接写 `updated_by_device_id != device_id`:
    SQL 三值逻辑下 `NULL != 'x'` 为 NULL(WHERE 视为假),旧版本服务端写入的
    行(不设该字段)被静默过滤,mobile 永远拉不到。
    """
    client, TS = _make_client()
    try:
        tok = _login(client, "pullnull@t.com")
        hdr = {"Authorization": f"Bearer {tok}"}

        # 正常 push 一条 user-scope(push 协议会填 updated_by_device_id=d1)
        _push(client, hdr, "lg1", "category", "c-own",
              {"syncId": "c-own", "name": "Own", "kind": "expense"})

        # 手工插入一条"旧版本写的历史行":updated_by_device_id = NULL
        with TS() as db:
            from src.models import User
            uid = db.scalar(select(User.id).where(User.email == "pullnull@t.com"))
            db.add(SyncChange(
                user_id=uid,
                ledger_id=None,
                scope="user",
                entity_type="category",
                entity_sync_id="c-legacy",
                action="upsert",
                payload_json={"syncId": "c-legacy", "name": "Legacy", "kind": "expense"},
                updated_at=datetime.now(timezone.utc),
                updated_by_device_id=None,
            ))
            db.commit()

        # 真实客户端行为:pull 恒带 device_id
        r = client.get("/api/v1/sync/pull?since=0&device_id=d1", headers=hdr)
        assert r.status_code == 200
        sync_ids = {c["entity_sync_id"] for c in r.json()["changes"]}
        assert "c-legacy" in sync_ids, "NULL updated_by_device_id 的历史行被 pull 过滤掉了"
        assert "c-own" not in sync_ids, "自己设备 push 的行仍应被过滤(原有契约不破)"
    finally:
        app.dependency_overrides.clear()
