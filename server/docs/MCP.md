...
让 LLM(Claude Desktop / Cursor / Cline 等)通过 [Model Context Protocol (MCP)](https://modelcontextprotocol.io) 直接读写你的 SmartBook 账本数据。

---

## 是什么

MCP 是 Anthropic 推出的 LLM-工具集成协议。SmartBook Cloud 内置一个 MCP server,把账本能力暴露成 18 个 tool:
...
- **7 个 write tool**:`create_transaction` / `create_transactions`(批量导入,一次提交多笔)/ `update_transaction` / `delete_transaction`(需二次确认)/ `create_category` / `update_budget` / `parse_and_create_from_text`(让 SmartBook AI 解析自然语言)
...
LLM 自动调对应 tool,你不用打开 SmartBook。MCP 创建的交易会自动打上 `MCP` 标签,跟手机端"AI 记账"区分。

---

## 启用步骤

### 1. 在 SmartBook Cloud Web 创建 PAT

1. 登录 SmartBook Cloud Web Console
...
SmartBook Cloud 的 MCP 用 **Streamable HTTP**(单端点),端点是部署地址加 `/api/v1/mcp`。

> 下面统一把占位符替换成真实值:
>
> - `https://your-domain.com` → SmartBook Cloud 部署地址(也可以是 `http://192.168.x.x:8080` 等内网/Tailscale 地址)
...
新开一个会话,`claude mcp get smartbook` 显示 `✔ Connected` 即成。其它带内置 HTTP MCP 客户端的 agent 同理:直接填端点 URL `https://your-domain.com/api/v1/mcp` + 请求头 `Authorization: Bearer bcmcp_…` 即可。
...
完全退出 Claude Desktop(`Cmd+Q`)再启动,左下角出现 🔌 "SmartBook" 即连上。
...
- 需要先在 Web 设置页配 AI provider(GLM / OpenAI 等)。这个 tool 是让 SmartBook 自己的 AI 解析,跟 LLM 客户端的 AI 不同
