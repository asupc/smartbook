# SmartBook Cloud MCP Server

Let LLM clients (Claude Desktop / Cursor / Cline / etc.) read and write your SmartBook ledger data via the [Model Context Protocol (MCP)](https://modelcontextprotocol.io).

---

## What it is

MCP is Anthropic's open standard for LLM tool integration. SmartBook Cloud ships a built-in MCP server exposing 17 tools:
...
- **6 write tools** — `create_transaction` / `update_transaction` / `delete_transaction` (two-step confirm) / `create_category` / `update_budget` / `parse_and_create_from_text` (let SmartBook's own AI parse free-form text)
...
The LLM picks the right tool, no need to open SmartBook. Transactions created via MCP are automatically tagged `MCP` to distinguish them from the mobile "AI bookkeeping" flow.

---

## Setup

### 1. Create a PAT in SmartBook Cloud Web

1. Log into the SmartBook Cloud web console
...
SmartBook Cloud's MCP uses **Streamable HTTP** (a single endpoint): your deployment URL plus `/api/v1/mcp`.

> Replace the placeholders below:
>
> - `https://your-domain.com` → your SmartBook Cloud URL (can also be `http://192.168.x.x:8080`, Tailscale, etc.)
...
```bash
claude mcp add --transport http smartbook https://your-domain.com/api/v1/mcp \
  --header "Authorization: Bearer bcmcp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
```

Open a new session; `claude mcp get smartbook` showing `✔ Connected` means it's set. Any client with a built-in HTTP MCP client works the same: point it at the endpoint URL `https://your-domain.com/api/v1/mcp` with an `Authorization: Bearer bcmcp_…` header.
...
Fully quit Claude Desktop (`Cmd+Q`) and relaunch — the 🔌 "SmartBook" indicator in the bottom-left means it's connected.
...
- You need to configure an AI provider (GLM / OpenAI / etc.) in the web settings first. This tool uses SmartBook's own AI to parse natural language — different from the LLM client's AI.
